...
public class StronglyConnectComp {
     ...
     int stronglyConnectedComponents(Graph graph){
        ...
        for (int i=0; i<graph.size();i++)
         ...
         Graph.Vertex ver;                  //For each element from stack run DFS for transpose graph
    ...
    static void dfsReverse(Graph.Vertex src, boolean[] visited, ArrayList<Graph.Vertex> list ){
        ...
        list.add(src);
        Iterator<Graph.Edge> itr = src.revAdj.iterator();       //Using revAdj to get transpose of a graph
        Graph.Vertex vertex ;
        while (itr.hasNext()){
