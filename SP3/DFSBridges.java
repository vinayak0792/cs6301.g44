...
public class DFSBridges extends GraphAlgorithm<DFSBridges.DFSVertex> {
	...
	public List<Edge> dfsVisit(Vertex u, int time, List<Edge> bridges) {
		...
		for (Edge e : u.adj) {
			...
			DFSBridges.DFSVertex dv = getVertex(v);
			if (!dv.seen) {
				children++;
				dv.parent = u;
				dfsVisit(e.to, time, bridges);
				// Checking if the anyone of the child has backEdges and
				// updating the low accordingly.
				du.low = Math.min(du.low, dv.low);

				// Condition to check if the edge from the current node to child
				// node is a bridge.
			...
			// Marking a node as cut vertex if it satisfies Rule one, i.e, if
			// the current node is a root node.
			if (du.parent == null && children > 1) {
				du.cutVertex = true;
			}
			// If the current node is not a root node checking if it is a
			// cutVertex based on whether anyone of its children have a back
			// edge.
